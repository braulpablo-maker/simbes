# CLAUDE.md — SIMBES · Configuración de Sesión

> Este archivo es leído automáticamente por Claude Code al iniciar cada sesión.
> Contiene dos secciones: (1) reglas de trabajo general y (2) contexto técnico del proyecto SIMBES.

---

## PARTE 1 — Orquestación del flujo de trabajo
...
- Si algo sale mal, DETENER y replantear de inmediato — no seguir adelante a ciegas
- Usar el modo planificación también para verificar, no solo para construir
- Redactar especificaciones detalladas desde el inicio para reducir la ambigüedad
...
- Para problemas complejos, escalar la capacidad de cómputo mediante subagentes
- Asignar una sola responsabilidad por subagente para mantener el foco

### 3. Ciclo de mejora continua
- Tras cualquier corrección del usuario: registrar el patrón en `tasks/lessons.md`
- Definir reglas propias que impidan repetir el mismo error
- Iterar sin piedad hasta reducir la tasa de errores
- Revisar los aprendizajes al inicio de cada sesión del proyecto

### 4. Verificación antes de cerrar
- Nunca declarar una tarea como terminada sin demostrar que funciona
- Comparar el comportamiento antes y después de los cambios cuando sea relevante
- Preguntarse: "¿Un ingeniero senior aprobaría esto?"
- Ejecutar pruebas, revisar logs y demostrar que la solución es correcta

### 5. Elegancia equilibrada
- Para cambios no triviales: hacer una pausa y preguntarse "¿existe una solución más elegante?"
- Si la solución parece un parche: "Con todo lo que sé ahora, implementar la solución correcta"
- Saltarse este paso para arreglos simples y obvios — no sobrediseñar
- Cuestionar el propio trabajo antes de presentarlo

### 6. Corrección autónoma de errores
- Al recibir un reporte de error: resolverlo directamente, sin pedir orientación
- Examinar logs, errores y pruebas fallidas — y luego resolverlos
- El usuario no debería tener que intervenir en el proceso de resolución
- Corregir pruebas de CI fallidas sin esperar instrucciones explícitas

### Gestión de tareas

1. **Planificar primero**: Escribir el plan en `tasks/todo.md` con ítems verificables
2. **Validar el plan**: Confirmar con el usuario antes de comenzar la implementación
3. **Registrar el avance**: Marcar los ítems como completados a medida que se avanza
4. **Explicar los cambios**: Incluir un resumen de alto nivel en cada paso
5. **Documentar resultados**: Agregar una sección de revisión en `tasks/todo.md`
6. **Capturar aprendizajes**: Actualizar `tasks/lessons.md` después de cada corrección

### Principios fundamentales

- **Simplicidad ante todo**: Cada cambio debe ser lo más simple posible. Tocar el mínimo de código necesario.
- **Sin atajos**: Encontrar la causa raíz. Nada de soluciones temporales. Estándares de desarrollador senior.
- **Impacto mínimo**: Los cambios solo deben afectar lo estrictamente necesario. Evitar introducir nuevos errores.

---

## PARTE 2 — Contexto del Proyecto SIMBES
