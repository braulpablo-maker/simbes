  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
        ...
        <span style={{ color: C.purple, fontWeight: 700, fontSize: 13, fontFamily: 'JetBrains Mono, monospace' }}>{Number(value).toFixed(dec)} <span style={{ color: C.muted, fontWeight: 400, fontSize: 11 }}>{unit}</span></span>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', color: C.muted, fontSize: 10, fontFamily: 'JetBrains Mono, monospace' }}><span>{min}</span><span>{max}</span></div>
    </div>
  );
}
...
function Metric({ label, value, unit, color = C.purple, sub }) {
  return (
    <div style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: 8, padding: '10px 14px', flex: 1, minWidth: 100 }}>
      ...
      <div style={{ color, fontWeight: 700, fontSize: 16, fontFamily: 'JetBrains Mono, monospace' }}>{value}</div>
      <div style={{ color: C.muted, fontSize: 10, fontFamily: 'JetBrains Mono, monospace' }}>{unit}{sub && <> · {sub}</>}</div>
    </div>
  );
}
...
function Alert({ type, msg }) {
  ...
  return <div style={{ background: s.bg, border: `1px solid ${s.bd}`, borderRadius: 8, padding: '10px 14px', marginBottom: 8, fontSize: 12, fontFamily: 'JetBrains Mono, monospace', color: s.c }}>{s.ic} {msg}</div>;
}
...
function GVFGauge({ GVF_wellbore_pct, GVF_pump_pct }) {
  ...
  return (
    <div style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: 8, padding: '12px 16px' }}>
      <div style={{ color: C.muted, fontSize: 10, fontFamily: 'JetBrains Mono, monospace', marginBottom: 10 }}>MEDIDOR GVF EN SUCCIÓN</div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: C.muted, fontFamily: 'JetBrains Mono, monospace', marginBottom: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 16, fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: '#CBD5E1' }}>
...
// ─── Tab Teoría ───────────────────────────────────────────────────
function TabTeoria() {
  return <TheoryLayout sections={TEORIA_M3} accentColor={C.ok} />;
}
  ...
  return (
    <div style={{display:'grid',gridTemplateColumns:'280px 1fr',gap:20}}>
      <div>
        ...
        <div style={{color:C.muted,fontSize:10,fontFamily:'JetBrains Mono, monospace',marginBottom:8,textTransform:'uppercase',letterSpacing:1}}>Fluido</div>
  ...
  return (
    <div>
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:'16px 20px',marginBottom:14}}>
        <div style={{color:C.purple,fontWeight:700,fontSize:13,fontFamily:'JetBrains Mono, monospace',marginBottom:8}}>{s.title}</div>
        <div style={{color:C.text,fontSize:13,lineHeight:1.6,marginBottom:10}}>{s.desc}</div>
        <div style={{background:C.surfaceAlt,borderRadius:6,padding:'8px 12px',color:C.blue,fontSize:12,fontFamily:'JetBrains Mono, monospace',marginBottom:8}}>📋 {s.task}</div>
        <div style={{color:C.muted,fontSize:11,fontFamily:'JetBrains Mono, monospace'}}>💡 {s.hint}</div>
      </div>
    ...
    <div>
      ...
      {submitted&&result&&(
        <div style={{background:result.pct>=80?'#052e16':result.pct>=60?'#431407':'#450a0a',border:`1px solid ${result.pct>=80?'#166534':result.pct>=60?'#92400e':'#991b1b'}`,borderRadius:10,padding:'16px 20px',marginBottom:20}}>
          <div style={{fontFamily:'JetBrains Mono, monospace',fontSize:18,fontWeight:700,color:result.pct>=80?C.ok:result.pct>=60?C.warning:C.danger}}>{result.score}/{result.total} correctas — {result.pct}%</div>
      ...
      {M3_QUESTIONS.map((q,qi)=>{
        ...
        return (
          <div key={q.id} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:'16px 20px',marginBottom:14}}>
            <div style={{color:C.text,fontWeight:700,fontSize:13,fontFamily:'JetBrains Mono, monospace',marginBottom:12}}>{qi+1}. {q.text}</div>
            {q.options.map(opt=>{
