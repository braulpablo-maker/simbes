...
import { M6_QUESTIONS, gradeM6, sampleQuestions as sampleM6 } from "../../../pedagogy/evaluations/m6";
...
function TabEvaluacion() {
  const [questions] = useState(() => sampleM6(5));
  const [answers, setAnswers] = useState({});
  const [result,  setResult]  = useState(null);
  const select = (qId, optId) => { if (!result) setAnswers(p => ({ ...p, [qId]: optId })); };
  const submit = () => {
    const r = gradeM6(questions.map(q => ({ id: q.id, selected: answers[q.id] || "" })));
...
export default function Module6({ onBack }) {
  ...
  return (
    <ModuleLayout
      ...
      subtitle="API RP 11S1 · Árbol de Fallas · Teardown Report · Prevención"
      accentColor={ACCENT}
      tabs={TABS}
      activeTab={tab}
      onTabChange={setTab}
